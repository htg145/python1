...
while True:
    # 난이도 입력 받음
    difficulty = input("난이도 를 선택해 주세요(쉬움, 보통, 어려움)")
    # 만약에 입력 받은 난이도 가 난이도 리스트 안에 있는 요소중 하나 이면
    if difficulty in difficulty_list:
        break
    else:
        # 난이도 리스트 에 없는 난이도 를 입력 했을 경우
        print("다시 난이도 를 입력해 주세요.")

# 난이도 선택 때 설정 되는 최대 시도 횟수, 최대 범위
if difficulty == "쉬움":
    max_try_count = 10
    max_range = 50
elif difficulty == "보통":
...
# 선택된 난이도 에 따라 랜덤 숫자 최대 범위는 달라 진다
...
# 게임 진행
    ...
    if correct_answer > guess:
        ...
        print(f"정답 입니다!! 정답 : {correct_answer}")
        break
